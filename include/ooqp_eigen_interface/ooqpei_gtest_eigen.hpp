...
namespace ooqpei { namespace eigen {
    ...
    void assertEqual(const M1 & A, const M2 & B, ooqpei::source_file_pos const & sfp, std::string const & message = "")
    {
    ...
    void assertNear(const M1 & A, const M2 & B, T tolerance, ooqpei::source_file_pos const & sfp, std::string const & message = "")
    {
    ...
    template<typename M1, typename M2, typename T>
    void expectNear(const M1 & A, const M2 & B, T tolerance, ooqpei::source_file_pos const & sfp, std::string const & message = "")
    {
    ...
    void assertFinite(const M1 & A, ooqpei::source_file_pos const & sfp, std::string const & message = "")
    {
      for(int r = 0; r < A.rows(); r++)
		{
		  for(int c = 0; c < A.cols(); c++)
      ...
      {																	\
		for(int c = 0; c < (A).cols(); c++)								\
		  {																\
			...
			ASSERT_TRUE(ooqpei::eigen::compareRelative( (A)(r,c), (B)(r,c), PERCENT_TOLERANCE, &percentError)) \
			  << MSG << "\nComparing:\n"								\
...
#define ASSERT_DOUBLE_SPARSE_MX_EQ(A, B, PERCENT_TOLERANCE, MSG)       \
  ...
  }} // namespace ooqpei::eigen

#endif /* OOQPEI_EIGEN_GTEST_HPP */
